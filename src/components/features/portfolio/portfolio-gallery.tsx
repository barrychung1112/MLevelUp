...
export function PortfolioGallery({
  ...
  onFiltersChange,
  status = "ready",
  errorMessage = "無法載入私人作品集。",
}: PortfolioGalleryProps) {
  const [internalFilters, setInternalFilters] = useState<PortfolioFilters>(
  ...
  const visibleArtifacts = privateArtifacts.filter((artifact) =>
    (activeFilters.artifactType === "all" || artifact.artifactType === activeFilters.artifactType) &&
    (activeFilters.skill === "all" || artifact.skillTags.includes(activeFilters.skill)),
  );
  ...
  if (artifacts.length === 0) return <section aria-labelledby="portfolio-heading" className="space-y-6">{header}<EmptyState title="私人作品集尚無成果" description="完成符合品質門檻的任務後，成果會出現在私人作品集中。" /></section>;
  if (privateArtifacts.length === 0) return <section aria-labelledby="portfolio-heading" className="space-y-6">{header}<EmptyState title="沒有可安全顯示的私人作品。" description="系統已隱藏未標記為私人的輸入資料。" /></section>;

  function changeFilters(nextFilters: PortfolioFilters) {
  ...
  function clearFilters() {
    changeFilters(DEFAULT_PORTFOLIO_FILTERS);
  }
    ...
    <section aria-labelledby="portfolio-heading" className="space-y-6">
      ...
      <fieldset className="command-panel grid gap-4 border border-command-border bg-command-surface/92 p-4 sm:grid-cols-2">
        ...
        <label className="grid gap-2 text-sm text-command-text">
          成果類型
          <select className={selectClass} value={activeFilters.artifactType} onChange={(event) => updateFilters({ artifactType: event.target.value })}>
        ...
        <label className="grid gap-2 text-sm text-command-text">
          能力標籤
          <select className={selectClass} value={activeFilters.skill} onChange={(event) => updateFilters({ skill: event.target.value })}>
