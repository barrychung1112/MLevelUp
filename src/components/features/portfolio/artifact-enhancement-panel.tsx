...
export function ArtifactEnhancementPanel(props: {
  ...
  function move(index: number, delta: -1 | 1) { if (!draft) return; const next = [...draft.bullets]; const target = index + delta; [next[index], next[target]] = [next[target], next[index]]; setDraft({ ...draft, status: "draft", bullets: next }); }
  ...
  return <div className="mt-4 space-y-3 border-t border-command-border pt-4">
    ...
    {draft ? <div className="space-y-2"><p className="text-xs text-command-muted">{draft.status === "approved" ? "Approved" : "Private draft"}</p>{draft.bullets.map((bullet, index) => <div key={bullet.id}><textarea aria-label={`Achievement ${index + 1}`} className="min-h-20 w-full border border-command-border bg-command-bg p-2 text-sm" maxLength={160} value={bullet.text} onChange={(event) => setDraft({ ...draft, status: "draft", bullets: draft.bullets.map((item) => item.id === bullet.id ? { ...item, text: event.target.value } : item) })} /><div className="flex gap-1"><Button size="sm" variant="secondary" disabled={index === 0} onClick={() => move(index, -1)}>Move up</Button><Button size="sm" variant="secondary" disabled={index === draft.bullets.length - 1} onClick={() => move(index, 1)}>Move down</Button><Button size="sm" variant="danger" disabled={draft.bullets.length <= 3} onClick={() => setDraft({ ...draft, status: "draft", bullets: draft.bullets.filter((item) => item.id !== bullet.id) })}>Remove</Button></div></div>)}<div className="flex gap-2"><Button size="sm" variant="secondary" loading={busy} onClick={() => void run(async () => { const result = await props.onUpdate(props.artifactId, "save", edits); setMessage(result.ok ? "Draft saved." : `Save failed: ${result.code}`); })}>Save draft</Button><Button size="sm" loading={busy} onClick={() => void run(async () => { const result = await props.onUpdate(props.artifactId, "approve", edits); if (result.ok) setDraft({ ...draft, status: "approved" }); setMessage(result.ok ? "Approved for portfolio." : `Approval failed: ${result.code}`); })}>Approve for portfolio</Button></div></div> : null}
