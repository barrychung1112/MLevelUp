...
export function TrainingArchive({ activities, eventTypes, status = "ready", errorMessage = "無法載入訓練紀錄。" }: TrainingArchiveProps) {
  const [eventFilter, setEventFilter] = useState("all");
  const options = eventTypes ?? [...new Set(activities.map((activity) => activity.eventType))].sort();
  const visibleActivities = activities.filter((activity) => eventFilter === "all" || activity.eventType === eventFilter);
  ...
  if (activities.length === 0) return <EmptyState title="Training Archive 尚無紀錄" description="完成任務後，戰鬥紀錄會出現在這裡。" />;
  ...
  return (
    <section aria-labelledby="archive-heading" className="space-y-6">
      ...
      <label className="grid max-w-sm gap-2 text-sm text-command-text">
        紀錄類型
        <select className={selectClass} value={eventFilter} onChange={(event) => setEventFilter(event.target.value)}>
      ...
      {visibleActivities.length === 0 ? (
        <EmptyState
          ...
          action={<Button variant="secondary" type="button" onClick={() => setEventFilter("all")}>清除紀錄篩選</Button>}
        />
      ) : (
