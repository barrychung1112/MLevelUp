...
export function AgentStatusBoard({ agents, status = "ready", errorMessage = "無法載入 Agent 狀態。" }: AgentStatusBoardProps) {
  ...
  if (agents.length === 0) return <EmptyState title="目前沒有 Agent 執行紀錄。" description="Phase 1 只會顯示模擬執行資料。" />;

  return (
    <section aria-labelledby="agents-heading" className="space-y-6">
      <header>
        <p className="text-sm uppercase tracking-[0.24em] text-command-warning">Simulation telemetry</p>
        <h1 id="agents-heading" className="text-3xl font-semibold text-command-text">Agent 狀態</h1>
        <p className="mt-2 text-command-muted">Phase 1 僅顯示模擬狀態，不會呼叫真實 AI 或排程。</p>
      </header>
      <ul className="grid gap-4 md:grid-cols-2">
        {agents.map((agent) => (
          <li key={agent.id} className="command-panel border border-command-border bg-command-surface/92 p-5">
            ...
            <p className="mt-3 text-sm text-command-muted">模擬執行時間：{agent.lastRun}</p>
          </li>
        ))}
      </ul>
    </section>
